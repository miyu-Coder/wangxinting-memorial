...
| AI 导览助手 | 金色悬浮按钮唤起对话弹窗，33 条预设问答覆盖将军生平/基地信息/系统操作，关键词分流匹配（功能类/知识类），详情页卡片引导入口 |
| 排行榜 | 答题排行榜，综合得分率(50%)+完成度(30%)+速度分(20%)，完成全部展点者优先 |
| 纪念品预约 | 展点满分预约对应奖品，四展点全满分解锁礼盒，排行榜预约按排名分配奖品，支持备注 |
...
| 功能 | 描述 |
|------|------|
| 数据看板 | 总访问量、今日/昨日 PV/UV、近 7 日趋势图（ECharts）、时段分布、转化率分析、Sankey 用户行为路径分析 |
...
| GET | `/api/quiz/user-records` | 查询用户各展点最高分（成就页同步） |
...
| GET | `/api/souvenir/check` | 查询预约状态（按昵称+类型） |
...
| GET | `/api/stats/user-flow` | 用户行为路径数据（Sankey 桑基图） |
...
│   └── utils.js               # 通用工具
├── js/
│   ├── common.js              # 全局昵称管理 + 庆祝动画 + 留言墙渲染 + AI 导览助手 + 悬浮按钮
│   ├── api.js                 # API 请求封装
│   ├── config.js              # 前端配置
│   ├── storage.js             # localStorage 封装
│   ├── ai-knowledge-base.js   # AI 知识库（功能操作/常见问答/引导回复）
